    ...
    <li class="dd-item dd3-item" data-id="{{ $item['id'] }}">
	       ...
	       <div class="dd3-content clearfix">
	          	...
	          	<p style="float:right; margin:0 !important;">
					<label class="menu__list__label">Show top: <input type="checkbox"
										   class="make-switch" data-size="mini" {{ !empty($item['view_top']) ? 'checked' : '' }}
										   data-on-text="<i class='fa fa-check'></i>"
										   data-off-text="<i class='fa fa-times'></i>"
										   onchange="Ajax.buttonView(this, '{{ $table }}', '{{ $item["id"] }}', 'view_top')"> </label>

					<label class="menu__list__label">Show bottom: <input type="checkbox"
										   class="make-switch" data-size="mini" {{ !empty($item['view_bottom']) ? 'checked' : '' }}
										   data-on-text="<i class='fa fa-check'></i>"
										   data-off-text="<i class='fa fa-times'></i>"
										   onchange="Ajax.buttonView(this, '{{ $table }}', '{{ $item["id"] }}', 'view_bottom')"> </label>
